...
//			если пробел, то смотрим с трeх сторон
			...
			{
				if (map_info->arr_map[i + 1] == '0' ||
					map_info->arr_map[i - 1] == '0' ||
					map_info->arr_map[i - map_info->max_line_len] == '0' ||
					map_info->arr_map[i + map_info->max_line_len] == '0' ||
					ft_isalpha(map_info->arr_map[i + 1]) ||
					ft_isalpha(map_info->arr_map[i - 1]) ||
					ft_isalpha(map_info->arr_map[i - map_info->max_line_len]) ||
					ft_isalpha(map_info->arr_map[i + map_info->max_line_len]))
				{
					write(2, "Error map: map is open\n", 23);
//удалить printf!!!
